...
// The old SearchOverlay content, minus the modal: no backdrop, no open or
// close state, no escape-to-close, no body scroll lock. It renders inline
// inside the Library tab, always mounted while that tab is active, and
...
export default function LibraryPanel() {
  ...
  // Read initial state from the URL (supports a future deep link like
  // ?tab=library&book=sapiens) and stay in sync with back and forward.
  useEffect(() => {
    const syncFromUrl = () => {
  ...
  // Focus the search field once, when the tab first mounts with no book open.
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
  ...
  return (
    <div className="library-panel" ref={panelRef} onScroll={handlePanelScroll}>
      <div className="wrap">
        ...
        {!selectedBook && matches.length === 0 && (
          <div className="results-empty">
            ...
            {/*
              TODO: the old empty state linked to #request, the landing
              page's request form. A signed-in user never sees that
              section anymore, so this needs a real destination, a
              request tab, a modal, or a mailto link, before shipping.
            */}
        ...
        {selectedBook && (
          <div className="book-detail">
            ...
            <aside className="angle-rail" ref={railRef} aria-label="Jump to a section">
              <div
            ...
            <div className="detail-main">
              <button className="back-link" type="button" onClick={backToResults}>
                &larr; All results
              </button>
              <div className="detail-head">
                <div className="detail-cover" style={{ background: selectedBook.cover }}>
